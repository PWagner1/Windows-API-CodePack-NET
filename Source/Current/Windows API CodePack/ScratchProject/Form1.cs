...
public partial class Form1 : Form
{
    ...
    private void TestShellThumbnailMarshaling()
    {
        try
        {
            ...
            MessageBox.Show(@$"Test path: {testPath}\nExists: {Directory.Exists(testPath)}", @"Debug Info", MessageBoxButtons.OK, MessageBoxIcon.Information);
            
            // Create a ShellObject from the path
            ShellObject? shellObject = ShellObject.FromParsingName(testPath);
            if (shellObject == null)
            {
                MessageBox.Show(@"Failed to create ShellObject from path.", @"Test Results", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            if (thumbnail == null)
            {
                MessageBox.Show(@"ShellObject has no Thumbnail.", @"Test Results", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
